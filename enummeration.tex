...
\subsection{Lower Bound for \( O(1) \) Memory}
...
\subsection{Lower bound for \( O(n) \)}
This one is obvious
\ref{eq:recurrence1}
\[
...
\subsection{Lower Bound for \( O(\log n) \) Memory}
...
\begin{aligned}
T(n_{\log n}(i), \log n)
  &= log(n) \binom{i + log(n) - 1}{log(n) + 1} + i \\
  &= log(n)  \frac{i-1}{log(n) +1}  \binom{i + log(n) - 1}{log(n)} + i\\
  &= log(n)  \frac{i-1}{log(n) +1}  n_{k}(i) + i\\
\end{aligned}
\]

Substituting the asymptotic orders knowing that $\frac{i-1}{log(n) + 1}$ tends to $1$ since $\theta(i) = log(n)$:
\[
T(n_{log(n)}(i), \log n) = log(n) \Theta(n) + \Theta(\log n) = \Theta(nlogn()).
\]
Hence, under this formal recurrence model, using \(\Theta(\log n)\) memory, the theoretical lower bound satisfies:
\[
T(n, \Theta(\log n)) = \Theta(nlog(n)).
\]

We now have a robust framework for the \(\Theta(\log n)\) memory.
\section{Algorithms Which Approach Our Theoretical Bounds}

\subsection{Algorithm for $O(n)$~\cite{filliatre2006backtracking}}
...
\subsection{Algorithms for \(O(\log n)\) Memory}
