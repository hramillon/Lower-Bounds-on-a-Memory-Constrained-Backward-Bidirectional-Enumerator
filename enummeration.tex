...
\[
n \sim \frac{(\log n + i)^{\log n}}{(\log n)!}
\quad \Longrightarrow \quad
i \sim \big( (\log n)! \cdot n \big)^{\frac{1}{\log n}} - \log n.
\]
...
we get
\[
i = \Theta\!\left( \big( n \cdot (\log n)^{\log n} \big)^{\frac{1}{\log n}} \right)
...
\paragraph{Choosing the precise constant.} Let \(i = c^* \log n\) where \(c^* \approx 0.5422\) solves
\[
(1+c^*)\ln(1+c^*) - c^* \ln c^* = 1.
\]
...
We have no a lower bound for $\theta(log(n))$
\section{Algorithms Which Approach Our Theoretical Bounds}

\subsection{Algorithm for $\theta(n)$ \cite{filliatre2006backtracking}}

This algorithm saves every position during forward traversal, enabling exact backward steps in constant time per operation.

\paragraph{Data Structures:} It maintains:
\begin{itemize}
    \item \texttt{current}: current enumerator state
    \item \texttt{distance}: distance from start
    \item \texttt{saved\_positions}: stack of previous positions
\end{itemize}

\paragraph{Initialization}
\begin{minted}[fontsize=\small, linenos=false, frame=single,breaklines]{python}
...
\paragraph{Forward Movement}
\begin{minted}[fontsize=\small, linenos=false, frame=single,breaklines]{python}
def next(self):
    self.distance += 1
    self.saved_positions.append(self.current
    .clone())
    self.current.next()
\end{minted}

\texttt{next} increments distance, saves the state, and advances the iterator. The underlying \texttt{current.next()} is defined externally:
...
\paragraph{Backward Movement}
\begin{minted}[fontsize=\small, linenos=false, frame=single,breaklines]{python}
def prev(self):
    if not self.saved_positions:
        raise ValueError("No saved positions")
    self.distance -= 1
    self.saved_positions.pop()
    self.current.prev()
\end{minted}

\texttt{prev} checks for saved states, pops the last, and moves backward. The underlying \texttt{current.prev()}:
...
\subsection{Algorithm for $\theta(\log n)$ }

We maintain sparse checkpoints at distances that are powers of two 
($1,2,4,8,\ldots$), which allows efficient reconstruction.  
However, this structure alone can be problematic, since a large number of 
elements may lie between two consecutive powers of two, for example 
between $256$ and $512$.

A natural idea is to subdivide the interval between two such powers by 
placing additional anchors.  
Returning to our example, the distance between $256$ and $512$ is $256$, 
and given our anchor set, the natural way to subdivide it is again using 
powers of two.  
We can then recursively apply the same idea between intermediate points, 
such as between $384 = 256 + 128$ and $512$.

In summary, our algorithm places anchors at every power of two, as well as 
at every power of two arising from recursively subdividing the intervals 
between two consecutive powers of two.  
The resulting hierarchy of anchors can be represented as a binary tree.

\begin{center} % centre tout le tikzpicture sur la page
\begin{tikzpicture}[
  level distance=1cm,
...
% Premier arbre
\begin{scope}[shift={(-3cm,0)}] % décalage à gauche
\node {2}
  child { node {4}
    child { node {6} }
    child { node {8} 
...
% Deuxième arbre
\begin{scope}[shift={(3cm,0)}] % décalage à droite
\node {2}
  child { node {4}
    child { node {6} }
    child { node {8} 
...
These trees have a recursive depth of~$2$: the powers of two appear at 
level~$0$; the subdivisions of $8$ into $10$ and $12$ form level~$1$; and 
the subdivisions of $12$ into $14$ and $16$ form level~$2$.

However, we only have $O(\log n)$ memory, i.e.\ enough to store the anchors 
along a single branch obtained by always following the right child from the 
root, since these nodes correspond to powers of two.

I propose several rules to keep the appropriate anchors in the tree.  
The algorithm is implemented in Python, and a link to the code is provided 
at the end of the paper.

First, note that the distance between $0$ and the next power of two is 
precisely that power of two.  
Since our memory is limited to the size of the branch obtained by repeatedly 
going right, the number of anchors at depth~$1$ in the recursion (i.e.\ 
children of a given level-$0$ node) equals the logarithm of the corresponding 
power of two minus~$1$:
\[
\lvert [2,4,8] \rvert \;=\; \lvert [8,10,12] \rvert.
\]
From this, we derive a rule for preserving the most useful anchors for 
performing the enumeration at any moment.

During the upward phase of the enumeration, if the cursor lies between two 
powers of two, we keep the earlier power of two for as long as possible.  
For example:
\[
[4,6,8] \;\Rightarrow\; [4,8,10] \;\Rightarrow\; [8,10,12].
\]
In general, as long as the anchor lies in the ``mid'' region between two 
powers of two, we always remove the anchor closest to the preceding power 
of two.

The following example would not actually occur yet, because we have not 
explained the rule for depth~$k$, though it follows the same principle:
\[
[8,10,12,16] \;\Rightarrow\; [8,12,16,18]
            \;\Rightarrow\; [8,16,18,20].
\]

At depth~$k$, we simply treat level $k-1$ as if it were level~$0$ and apply 
the same rules recursively:
\[
[16,18,20,22] \;\Rightarrow\; [16,18,20,24]
              \;\Rightarrow\; [16,20,24,26].
\]

\begin{table}[H]
\centering
\caption{Experimental values of the cost for several $(n,k)$ pairs.}
\label{tab:experimental-values}
\medskip
\begin{tabular}{c|c|c|c|c}
\toprule
$n$ & $k$ & Operations & Theoretical bound & Ratio \\
\midrule
$16$  & $4$ & $80$   & $64$   & $1.250$ \\
$64$  & $6$ & $476$  & $384$  & $1.240$ \\
$256$ & $8$ & $2496$ & $2048$ & $1.219$ \\
$4096$  & $12$ & $56822$  & $48828$  & $1.164$ \\
\bottomrule
\end{tabular}
\end{table}
...
\multirow{4}{*}{$10$}
 ...
 & $128$  & $128$  & $208$  & $0.616$ \\
 & $512$  & $512$  & $955$  & $0.536$ \\
 & $2048$ & $2048$ & $4390$ & $0.467$ \\
\midrule
\multirow{4}{*}{$50$}
 & $32$   & $32$   & $34$   & $0.933$ \\
 & $128$  & $128$  & $141$  & $0.908$ \\
 & $512$  & $512$  & $580$  & $0.883$ \\
 & $2048$ & $2048$ & $2385$ & $0.859$ \\
\midrule
\multirow{4}{*}{$100$}
 & $32$   & $32$   & $33$   & $0.966$ \\
 & $128$  & $128$  & $134$  & $0.953$ \\
 & $512$  & $512$  & $545$  & $0.940$ \\
 & $2048$ & $2048$ & $2210$ & $0.927$ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{align*}
x_k(i+1) &= n_k(i), \\
x_k(i) - n_k(i) - 1 &= n_{k-1}(i), \\
x_k(i+1) &= x_k(i) + x_{k-1}(i), \\
T(n_k(i), k) &= k \times \binom{k + i}{k + 1}.
\end{align*}
...
\[
\Omega\left(n^{1 + \frac{1}{k}}\right), \quad \Omega(n), \quad \text{and} \quad \Omega(n \log n).
\]

Finally, we proposed algorithms that approach these bounds, suggesting that our lower bounds are tight—or nearly tight—in practice. These algorithms were inspired by prior work on checkpointing and external memory by Gerth St{\o}lting Brodal and Rolf Fagerberg \cite{brodal1999kk}, by Jeffrey Scott Vitter \cite{vitter1999external}, and by backtracking mechanisms developed by Jean-Christophe Filli{\^a}tre \cite{filliatre2006backtracking}.

In addition, we provided a straightforward proof for an upper bound of $\mathcal{O}(n^2)$, which complements the lower bounds and further validates the tightness of our complexity analysis.
